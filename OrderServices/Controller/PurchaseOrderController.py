...
from OrderServices.models import PurchaseOrder, PurchaseOrderItems, PurchaseOrderLogs
...
class PurchaseOrderSerializer(serializers.ModelSerializer):
    items = PurchaseOrderItemSerializer(
        many=True,
        source="po_id_purchase_order_items"
        )
    class Meta:
        model=PurchaseOrder
        fields="__all__"
    ...
    def create(self, validated_data):
        ...
        purchaseOrderLog = PurchaseOrderLogs(
            po_id=purchaseOrder,
            additional_details=[{"key":"STATUS","value":validated_data.get("status")}],
            comment="Purchase Order Created with Status: "+validated_data.get("status"),
            created_by_user_id=validated_data.get("created_by_user_id"),
            domain_user_id=validated_data.get("domain_user_id")
        )
    ...
    def update(self, instance, validated_data):
        ...
        for item_data in items_data:
            ...
            if 'id' in item_data:
                poItem=PurchaseOrderItems.objects.filter(
                    id = item_data.get('id')
                )
                po_item_item_serializer = PurchaseOrderItemSerializer(
                    poItem.first(),
                    data=item_data
                    )
                if po_item_item_serializer.is_valid():
                    po_item_item_serializer.save()
            else:
                PurchaseOrderItems.objects.create(
                po_id=instance,
                **item_data
...
class PurchaseOrderListSerializer(serializers.ModelSerializer):
    ...
    updated_by_user_id=serializers.CharField(source="updated_by_user_id.username", read_only=True)
...
class CreatePurchaseOrderView(generics.CreateAPIView):
    ...
    def get(self, request,id=None):
        ...
        poItemFields = getDynamicFormFields(
            PurchaseOrderItems(),
            request.user.domain_user_id.id,
            skip_related=['product_id'],
            skip_fields=[
                'quantity_received', 'quantity_cancelled', 'quantity_returned',
                'amount_returned', 'amount_cancelled', 
                'shipping_cancelled_amount',
                'shipping_cancelled_tax_amount', 'approved_at', 'cancelled_at',
                'received_at', 'returned_at', 'po_id', 'status',
                'approved_by_user_id', 'cancelled_by_user_id',
                'received_by_user_id', 'returned_by_user_id', 
                'cancelled_reason'
            ]
        )
    ...
    def post(self, request, id=None):
        ...
        if id:
            ...
            if po:
                ...
                return renderResponse(message="Purchase order Not Found", status=404)
        else:
